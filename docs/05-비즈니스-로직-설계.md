...
│   ├── PointBalanceReconciliationService  # 잔액 정합성 보정 서비스
│   └── PointBalanceCacheUpdateService     # 잔액 캐시 업데이트 서비스 (재시도 포함)
└── event/
    ├── PointBalanceEventHandler           # 잔액 이벤트 핸들러 (비동기)
    └── BalanceReconciliationEventHandler  # 잔액 보정 요청 이벤트 핸들러
```

### 2.2 이벤트 기반 잔액 관리

포인트 거래(적립/사용/취소) 발생 시 `MemberPointBalance` 캐시 테이블을 비동기로 업데이트합니다.

```
[포인트 서비스] ─── 트랜잭션 커밋 ───→ [이벤트 발행] ─── 비동기 ───→ [PointBalanceEventHandler]
                                                                              │
                                                                              ↓
                                                    ...
                                                    [성공]                                  [실패 → 재시도]
                                                        │                                           │
                                                        ↓                                    (최대 3회)
                                            [MemberPointBalance 업데이트]                           │
                                                                                                    ↓
                                                                                          [최종 실패 시]
                                                                                                    │
                                                                                                    ↓
                                                                            [BalanceReconciliationRequestEvent 발행]
                                                                                                    │
                                                                                                    ↓
                                                                            [BalanceReconciliationEventHandler]
                                                                                                    │
                                                                                                    ↓
                                                                            [SUM 쿼리로 정확한 잔액 계산 → 캐시 보정]
```

- **@TransactionalEventListener(AFTER_COMMIT)**: 원본 트랜잭션 커밋 후 이벤트 처리
- **@Async**: 비동기 실행으로 메인 응답 시간에 영향 없음
- **캐시 테이블**: 빠른 잔액 조회를 위한 O(1) 조회 지원

#### 2.2.1 재시도 및 복구 매커니즘

캐시 업데이트 실패에 대비한 재시도 및 복구 로직이 구현되어 있습니다.
...
class PointBalanceCacheUpdateService(
     ...
     */
    @Recover
    fun recoverFromFailure(e: Exception, event: PointBalanceEvent, action: (MemberPointBalance) -> Unit) {
...
> 📌 자세한 내용은 [09. 향후 개선 사항](./09-향후-개선-사항.md)의 "Transactional Outbox Pattern" 섹션을 참조하세요.
 ...
 * - READ_COMMITTED 적용하여 Dirty-Read를 방지하면서도, 과도한 격리로 인한 성능 저하 방지
 */
...
- 우선순위에 따른 적립 건 선택
- 포인트 사용 및 적립 건별 상세 내역 생성
- 적립 건의 사용 가능 잔액 업데이트 (배치 처리)

**성능 최적화**:
- **상세 내역 저장 최적화**: 1원 단위 개별 저장 → 적립 건당 1개 레코드로 개선
  - 10,000원 사용 시: 10,000개 → 1개 레코드 (99.99% 감소)
  - DB INSERT, 메모리, 스토리지 비용 대폭 절감
- 적립 건 저장은 `saveAll()`을 통한 배치 처리로 성능 향상
- 함수형 프로그래밍 스타일(`fold`)을 활용한 불변성 유지
- JPA 배치 설정을 통한 네트워크 왕복 횟수 감소
...
#### 6.1.1 잔액 조회 전략 (하이브리드)

| 조회 방식 | 사용 시점 | 특징 |
|----------|----------|------|
| **MemberPointBalance 캐시** | 일반 잔액 조회 | O(1) 빠른 조회, 비동기 업데이트 |
| **SUM 쿼리 (Fallback)** | 캐시 없는 경우 | 정확한 계산, 적립 건 직접 합산 |
...
class PointQueryService(
    private val pointAccumulationPersistencePort: PointAccumulationPersistencePort,
    private val pointUsagePersistencePort: PointUsagePersistencePort,
    private val memberPointBalancePersistencePort: MemberPointBalancePersistencePort  // 캐시 조회용
) : PointQueryUseCase {
...
- **사용**: 사용 건 저장, 상세 내역 저장, 적립 건 일괄 저장 (배치 처리)
