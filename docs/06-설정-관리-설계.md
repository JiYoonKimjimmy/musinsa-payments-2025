...
class PointConfigService(
    ...
    private val configCache: Cache<String, PointConfig>
) {
    
    @PostConstruct
    fun initializeCache() {
        // 애플리케이션 시작 시 모든 설정을 캐시에 로드
        val configs = pointConfigPort.findAll()
        configs.forEach { config ->
            configCache.put(config.configKey, config)
        }
    }
    
    fun getConfig(configKey: String): PointConfig {
        // 캐시에서 조회, 없으면 DB에서 조회 후 캐시에 저장
        return configCache.get(configKey) {
            pointConfigPort.findByConfigKey(configKey)
                .orElseThrow { ConfigNotFoundException(configKey) }
        }
    }
    
    @CacheEvict(value = ["pointConfig"], key = "#configKey")
    fun updateConfig(configKey: String, configValue: String) {
        // 설정 업데이트 시 캐시 무효화
...
PointConfigService.updateConfig()
    ↓
ConfigValidator.validateConfigValue()      ← 개별 값 검증
    ↓
ConfigValidator.validateConfigDependencies() ← 설정 간 의존성 검증
    ↓
PointConfigPort.save()                     ← DB 저장
    ↓
@CacheEvict                                ← 캐시 무효화
    ↓
PointConfigHistoryRepository.save()        ← 변경 이력 기록
    ↓
UpdateConfigResponse 응답
```

**캐시 무효화 위치**: `PointConfigService.updateConfig()` 메서드에서 `@CacheEvict` 어노테이션을 통해 저장 직후 자동으로 수행됩니다.
...
class PointConfigValidator {
    ...
    fun validateConfigValue(configKey: String, configValue: String) {
        when (configKey) {
            ...
            "MAX_EXPIRATION_DAYS" -> {
                ...
                throw InvalidConfigKeyException(configKey)
            }
        }
    }
    ...
    private fun validateLongValue(value: String, min: Long, max: Long) {
        val longValue = value.toLongOrNull()
            ?: throw InvalidConfigValueException("설정 값은 숫자여야 합니다.")
        
        if (longValue < min || longValue > max) {
            throw InvalidConfigValueException(
                "설정 값은 $min 이상 $max 이하여야 합니다.")
        }
    }
...
class PointConfigInitializer(
    ...
    private fun initializeConfigIfNotExists(
        configKey: String,
        defaultValue: String,
        description: String
    ) {
        if (!pointConfigPort.findByConfigKey(configKey).isPresent) {
...
class PointAccumulationService(
    private val configService: PointConfigService
) {
    
    fun accumulate(request: AccumulatePointRequest) {
        // 설정 조회
        val maxAmount = configService
...
class PointConfigHistory {
    ...
    var changedBy: String? = null  // 변경한 사용자
