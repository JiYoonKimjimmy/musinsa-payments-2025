...
class PointAccumulationTest {
    ...
    fun canBeCancelled_whenNotUsed_returnsTrue() {
        // given
    ...
    fun canBeCancelled_whenPartiallyUsed_returnsFalse() {
        // given
        val accumulation = PointAccumulation().apply {
            amount = Money.of(1000)
    ...
    fun use_decreasesAvailableAmount() {
        // given
        val accumulation = PointAccumulation().apply {
            amount = Money.of(1000)
            availableAmount = Money.of(1000)
        ...
        // then
        assertThat(accumulation.availableAmount).isEqualTo(Money.of(700))
    }
    
    @Test
    @DisplayName("포인트 사용 - 사용 가능 잔액 부족 시 예외 발생")
    fun use_whenInsufficientAmount_throwsException() {
        // given
        val accumulation = PointAccumulation().apply {
            amount = Money.of(1000)
            availableAmount = Money.of(500)
    ...
    fun isExpired_whenExpirationDatePassed_returnsTrue() {
        // given
        val accumulation = PointAccumulation().apply {
...
class MoneyTest {
    ...
    @Test
    @DisplayName("금액 뺄셈")
    fun subtract_returnsDifference() {
        // given
        val money1 = Money.of(1000)
    ...
    fun of_whenNegativeAmount_throwsException() {
        // when & then
...
class PointUsagePriorityServiceTest {
    ...
    private lateinit var service: PointUsagePriorityService
    
    @BeforeEach
    fun setUp() {
        service = PointUsagePriorityService()
    }
    
    @Test
    @DisplayName("수기 지급 포인트가 우선 사용됨")
    fun selectAccumulationsForUsage_manualGrantFirst() {
        // given
        ...
        assertThat(selected[0].isManualGrant()).isTrue()  // 수기 지급이 먼저
    }
    
    @Test
    @DisplayName("만료일이 짧은 포인트가 우선 사용됨")
    fun selectAccumulationsForUsage_expirationDateAscending() {
        // given
        ...
        // then
        assertThat(selected).hasSize(2)
        assertThat(selected[0].expirationDate)
            .isBefore(selected[1].expirationDate)
    }
}
```
...
class PointAccumulationPersistenceAdapterTest {
    ...
    @Autowired
    private lateinit var jpaRepository: JpaPointAccumulationRepository
    
    private lateinit var adapter: PointAccumulationPersistenceAdapter
    private lateinit var mapper: PointEntityMapper
    ...
    fun saveAndFind() {
        ...
        assertThat(found?.amount).isEqualTo(Money.of(1000))
    }
}
```
...
class PointAccumulationServiceIntegrationTest {
    ...
    @Autowired
    private lateinit var service: PointAccumulationService
    
    @Autowired
    private lateinit var configRepository: PointConfigRepository
    
    @Test
    @DisplayName("포인트 적립 전체 플로우")
    fun accumulate_fullFlow() {
        // given
        ...
        assertThat(response.availableAmount).isEqualTo(1000L)
    }
}
```
...
class Scenario1Test {
    ...
    @Autowired
    private lateinit var accumulationService: PointAccumulationService
    
    @Autowired
    private lateinit var usageService: PointUsageService
    
    @Test
    fun scenario1_basicAccumulationAndUsage() {
        ...
        // 검증: 총 잔액 1000원
        assertThat(response1.amount).isEqualTo(1000L)
        assertThat(response1.availableAmount).isEqualTo(1000L)
...
class Scenario2Test {
    ...
    @Test
    fun scenario2_expiredPointCancellation() {
        // 1. 포인트 적립 및 사용
        // ...
