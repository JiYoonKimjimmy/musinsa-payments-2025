...
**테스트 케이스 예시** (Kotest Behavior Spec 스타일 - BDD):
...
class PointUsagePriorityServiceTest : BehaviorSpec({
    ...
    Given("수기 지급 포인트와 만료일이 다른 일반 포인트들이 있을 때") {
        val manualGrantLongExpiration = PointAccumulation().apply {
            id = 1L
            amount = Money.of(2000)
            availableAmount = Money.of(2000)
            isManualGrant = true
            expirationDate = LocalDate.now().plusDays(365)
        }
        
        val normalShortExpiration = PointAccumulation().apply {
            id = 2L
            amount = Money.of(500)
            availableAmount = Money.of(500)
            isManualGrant = false
        ...
        val normalLongExpiration = PointAccumulation().apply {
            id = 3L
            amount = Money.of(500)
            availableAmount = Money.of(500)
            isManualGrant = false
        ...
        val accumulations = listOf(
            normalLongExpiration,
            normalShortExpiration,
            manualGrantLongExpiration
        )
        val usageAmount = Money.of(2500)
        
        When("포인트 사용 우선순위를 선택하면") {
            val selected = service
                .selectAccumulationsForUsage(1L, usageAmount, accumulations)
            
            Then("수기 지급 포인트가 최우선으로 선택되고, 이후 만료일이 짧은 포인트가 선택되어야 한다") {
                selected shouldHaveSize 3
                selected[0].isManualGrant() shouldBe true
                selected[1].expirationDate shouldBeLessThan selected[2].expirationDate
            }
        }
    }
})
...
- **Kotest**: Kotlin 전용 테스트 프레임워크
  - 다양한 테스트 스타일 지원 (String Spec, Behavior Spec, Fun Spec 등)
    - **String Spec**: 간단한 테스트 케이스 작성에 적합
    - **Behavior Spec**: BDD 스타일의 Given-When-Then 구조로 도메인 서비스 테스트에 적합
    - 시나리오 기반 테스트 작성 시 가독성 향상 및 비즈니스 요구사항 표현 용이
  - 내장 어설션 라이브러리
...
| 지표 | 목표값 | 측정 방법 |
|------|--------|-----------|
| **TPS** (Transaction Per Second) | 1,000 TPS 이상 | 포인트 적립/사용 API |
| **응답 시간 (평균)** | 100ms 이하 | P50 기준 |
| **응답 시간 (95%)** | 300ms 이하 | P95 기준 |
| **에러율** | 0.1% 이하 | HTTP 5xx 응답 비율 |
...
- **k6**: 경량 부하 테스트 도구
- **JMeter**: 대규모 부하 테스트 (옵션)
- **Gatling**: Scala 기반 부하 테스트 (옵션)
...
export default function () {
    ...
    const response = http.post(
        'http://localhost:8080/api/points/accumulate',
        payload,
...
- **목적**: 시스템 한계점 파악
- **방법**: 점진적 부하 증가 후 실패 지점 확인
- **지표**: 최대 동시 사용자 수, 시스템 복구 시간
